...
G4VPhysicalVolume* GaletDetectorConstruction::DefineVolumes(){
  ...
  //
  phantom->ReadVTK("Dicom5A.vtk");
  // --Visualization
  phantom->SetVisOn(true);
  G4LogicalVolume* phantomLV = phantom->ConstructPhantom();
  G4RotationMatrix* rotation = new G4RotationMatrix();
  rotation->rotateX(90.*degree); rotation->rotateZ(180.*degree); //rotation->rotateY(180.*degree);
  G4ThreeVector position(0,0,-125.*mm); //G4ThreeVector& position = phantom->GetCenterCoord();
  //
